...
#     return f"""
# You are helping create a Social Story for a specific child.
# Use the Child Profile below to adapt your tone and complexity.
# Use the Situation Details to understand the context.
#
# Ask ONE short, clear follow-up question to better personalize the story.
#
# CONTEXT AND DATA:
# {context}
#
...
#     """
#     return f"""
# You are an expert consultant helping a parent create a Social Story for their child.
# Your goal is to gather the missing information needed to write an effective story.
#
# To write a valid Social Story, you need clear answers to these 6 questions.
# Analyze the CONTEXT below and check what is missing:
# 1. WHO is involved?
...
# Use the Situation Details to understand the context.
#
# Identify the MOST CRITICAL GAP in the information. (What do you need to know most to write the story?)
# Ask ONE short, clear follow-up question to fill the gap or to better personalize the story.
#
#
# CONTEXT AND DATA:
# {context}
...
def show_login():
    ...
    with col_img_left:
        ...
        try:
            st.image("images/mother_child.png", use_container_width=True)
        except:
    ...
    with col_img_right:
        ...
        try:
            st.image("images/books_stack.png", use_container_width=True)
        except:
...
def show_Manage_Profiles():
    ...
    with col_buttons:
        ...
        for index, row in active_kids.iterrows():
            if st.button(row['Full Name'], key=f"select_{row['ID']}", use_container_width=True):
        ...
        if st.button("Add a New Profile", use_container_width=True):
            navigate_to('new_profile')
            st.rerun()

    with col_image:
        try:
            st.image("images/children_playing.png", use_container_width=True)
        except:
...
def show_new_profile():
    ...
    with st.form("create_profile_form", clear_on_submit=False):
        ...
        with c_image:
            ...
            try:
                st.image("images/boy.png", use_container_width=True)
            except:
        ...
        with c_input_left:
            ...
            with sub_c2:
                st.markdown("", help="General level of independence and support in everyday activities.\nHigh: Independent.\nModerate: Needs some support.\nLow: Needs full support.")
            st.markdown("<br>", unsafe_allow_html=True)
        ...
        with c_input_right:
            ...
            with sub_c1:
                comp_level = st.selectbox("Comprehension Level",
                                          ...
                                          label_visibility="collapsed")
            with sub_c2:
                st.markdown("", help="How well your child understands language and instructions. Literal: Concrete understanding.\nAbstract: Understands metaphors/nuance.")
            st.markdown("<br>", unsafe_allow_html=True)
            ...
            with sub_c1:
                read_level = st.selectbox("Reading Level", ["Reading Level", "Pre-reader", "Early Reader", "Fluent"],
                                          label_visibility="collapsed")
            with sub_c2:
                st.markdown("", help="Pre: Cannot read.\nEarly: Simple words.\nFluent: Reads sentences/books.")
            st.markdown("<br>", unsafe_allow_html=True)
            ...
            with sub_c2:
                st.markdown("", help="Favorite toys, topics, or characters.")
            st.markdown("<br>", unsafe_allow_html=True)
            ...
            with sub_c1:
                strengths = st.text_input("Strengths & Motivators", placeholder="Strengths & Motivators",
                                          label_visibility="collapsed")
            with sub_c2:
                st.markdown("", help="What motivates them? (Praise, tokens, etc.)")
...
def show_child_profile():
    ...
    with c_left:
        ...
        try:
            st.image("images/reading_child.png", use_container_width=True)
        except:
    ...
    with c_center:
        ...
        if st.button("Create New Story", type="secondary", use_container_width=True):
            navigate_to('create_new_story')
            st.rerun()

        # Button B: Saved Stories -> Goes to Saved Story View
        if st.button("Saved Stories", type="secondary", use_container_width=True):
            navigate_to('saved_stories_list')
            st.rerun()

        # Button C: Update Profile
        if st.button("Update Profile Details", type="secondary", use_container_width=True):
    ...
    with c_right:
        ...
        with d_col2:
            ...
            if not st.session_state.confirm_profile_delete:
                ...
                if st.button("Delete Profile", key="delete_profile_init", type="primary", use_container_width=True):
            ...
            # State B: Confirmation State
                ...
                with col_yes:
                    # Yes = Red (Primary)
                    if st.button("Yes", key="del_prof_yes", type="primary", use_container_width=True):
                ...
                with col_cancel:
                    ...
                    if st.button("Cancel", key="del_prof_cancel", use_container_width=True):
                        st.session_state.confirm_profile_delete = False
                        st.rerun()
...
def show_create_new_story():
    ...
    # --- TITLE ---
    st.markdown(
        "<h1 style='text-align:center;'>Create New Story</h1>",
        unsafe_allow_html=True
    )
    st.markdown(
        "<h3 style='text-align:center;'>Ready-to-use Templates for your convenience</h3>",
        unsafe_allow_html=True
    )
    ...
    with col1:
        st.image("images/New_School.png", use_container_width=True)
        if st.button("Starting a New School", use_container_width=True):
            st.session_state.selected_template = "new_school"
            navigate_to("new_school_story")
            st.rerun()

    with col2:
        st.image("images/New_Pet.png", use_container_width=True)
        if st.button("Getting a New Pet", use_container_width=True):
            st.session_state.selected_template = "new_pet"
            navigate_to("new_pet_story")
            st.rerun()

    with col3:
        st.image("images/New_Baby.png", use_container_width=True)
        if st.button("A New Baby Sibling", use_container_width=True):
    ...
    with custom_col2:
        if st.button("Create a Custom Story", use_container_width=True):
...
def show_new_school_story():
    ...
    with btn_col2:
        if st.button("Generate A Story", use_container_width=True):
...
def show_new_pet_story():
    ...
    with right_col:
        pet_type_input = st.text_input("", placeholder="Type of Pet (e.g. Dog, Cat)", key="pet_type")
        st.markdown("<br>", unsafe_allow_html=True)

        pet_name_input = st.text_input("", placeholder="Pet’s Name", key="pet_name")
        st.markdown("<br><br>", unsafe_allow_html=True)

        st.markdown("### Anything else you’d like to add?")
        additional_notes = st.text_area("", height=260, key="pet_additional_notes",
                                        placeholder="e.g. The dog is very big but friendly...")
...
def show_additional_details_1():
    ...
    st.markdown(
        "<h3 style='text-align:center; font-style: italic;'>"
        "To adjust the story especially for you, we just need a few more details:"
        "</h3>",
        unsafe_allow_html=True
    )
    st.markdown("<br><br>", unsafe_allow_html=True)
    ...
    if not st.session_state.is_thinking:
        ...
        with btn_c:
            next_clicked = st.button("Next Question", use_container_width=True)
            if next_clicked:
        ...
        with bottom_r:
            if st.button("Skip All & Generate A Story", use_container_width=True):
...
def show_story_display():
    ...
    with c_like:
        ...
        if st.button("Like!", key="btn_like_story", use_container_width=True):
    ...
    with c_dislike:
        ...
        if st.button("Dislike", key="btn_dislike_story", use_container_width=True):
...
def show_like_story():
    ...
    with c_save:
        if st.button("Save", key="btn_save_finished", use_container_width=True):
    ...
    with c_discard:
        if not st.session_state.confirm_delete_finished:
            if st.button("Discard", key="btn_discard_finished", use_container_width=True):
            ...
            col_yes, col_cancel = st.columns(2)
            with col_yes:
                if st.button("Yes", key="confirm_yes_finished", type="primary", use_container_width=True):
                    st.success("Deleted!")
                    time.sleep(1)
                    st.session_state.confirm_delete_finished = False
                    navigate_to('child_profile')
                    st.rerun()
            with col_cancel:
                if st.button("Cancel", key="confirm_cancel_finished", use_container_width=True):
                    st.session_state.confirm_delete_finished = False
                    st.rerun()
    ...
    with col_img:
        try:
            st.image("images/boy2.png", use_container_width=True)
        except:
...
def show_dislike_story():
    ...
    with c_regen:
        if st.button("Regenerate Story", key="btn_regen", use_container_width=True):
    ...
    # --- Save Button ---
    with c_save:
        if st.button("Save Story", key="btn_save_feedback", use_container_width=True):
    ...
    with c_discard:
        if not st.session_state.confirm_delete:
            if st.button("Discard Story", key="btn_discard_feedback", use_container_width=True):
            ...
            col_yes, col_cancel = st.columns(2)
            with col_yes:
                if st.button("Yes", key="confirm_yes", type="primary", use_container_width=True):
                    st.success("Deleted!")
                    time.sleep(1)
                    st.session_state.confirm_delete = False
                    navigate_to('child_profile')
                    st.rerun()
            with col_cancel:
                if st.button("Cancel", key="confirm_cancel", use_container_width=True):
                    st.session_state.confirm_delete = False
                    st.rerun()
...
def show_saved_story_display():
    ...
    with c_center:
        if st.button("Back", key="btn_back_bottom", use_container_width=True):
            st.session_state.confirm_remove_saved = False
            navigate_to('saved_stories_list')
            st.rerun()
    ...
    with c_right:
        if not st.session_state.confirm_remove_saved:
            if st.button("Remove Story", key="btn_remove_init", type="primary", use_container_width=True):
            ...
            with sub_yes:
                if st.button("Yes", key="btn_remove_yes", type="primary", use_container_width=True):
                    # --- DELETE LOGIC ---
            ...
            with sub_cancel:
                if st.button("No", key="btn_remove_cancel", use_container_width=True):
                    st.session_state.confirm_remove_saved = False
                    st.rerun()
...
def show_saved_stories_list():
    ...
    if not child_stories:
        ...
        with col2:
            ...
            if st.button("Create New Story", use_container_width=True, type="primary"):
                navigate_to('create_new_story')
                st.rerun()
    ...
    # CASE B: STORIES EXIST (Show List of Buttons)
        ...
        for story in child_stories:
            ...
            with c_btn:
                ...
                if st.button(btn_label, key=f"story_btn_{story['id']}", use_container_width=True):
    ...
    with b_center:
        if st.button("Back to Profile", key="btn_back_main", type="secondary", use_container_width=True):
            navigate_to('child_profile')
            st.rerun()
